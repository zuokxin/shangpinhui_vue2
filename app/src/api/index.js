...
// 封装函数：复用
// 将来这个函数可以用在别的地方使用，需要对外暴露【分别暴露】
// 获取商品分类的数据
export const reqCategory = () => {
    // 箭头函数可以在程序任意地方使用，箭头函数返回即为服务器的数据
    // 箭头函数的返回值：返回的是promise，即为服务器的数据
    // return关键字，不写的话默认返回的是undefined
...
// 请求头携带搜索的参数
// 搜索的条件：他应该是一个对象10k，可有可无，但是data至少是一个空对象
// 没有发现：
// 第一步：整理搜索的参数
// 第二步：根据最新的搜索参数，获取最新的数据展示
export const reqSearchList = data => requests({ url: '/list', methods: 'post', data });
